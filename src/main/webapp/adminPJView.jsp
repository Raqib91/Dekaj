...
<body>
	...
	<%
		...
		ArrayList<Job> pj = avd.viewPJobData("pending_job");
		
		AbstractApplicationContext appContext1 = new AnnotationConfigApplicationContext(EditRecruiterDao.class);
		EditRecruiterDao erd = appContext1.getBean(EditRecruiterDao.class);
// 		String str = erd.comName(mail);
	...
	<script type="text/javascript">
		$(function() {
			<%for (int i = 0; i < pj.size(); i++) {%>
				...
				$('.inner').append('<tr><td>' + "<%=i + 1 %>" + '</td><td>' + "<%=erd.comName(pj.get(i).getJobCom()) %>" + '</td><td>' + "<%=pj.get(i).getJobTitle() %>" + '</td><td><form action="approvePJ" method="post"><input type="hidden" name="comName" value="<%=pj.get(i).getJobCom() %>"><input type="hidden" name="comJob" value="<%=pj.get(i).getJobTitle() %>"><button type="submit" class="btn btn-info btn1"><i class="fa fa-sign-in"></i>&nbsp Approve</button></form><form action="rejectPJ" method="post"><input type="hidden" name="comName" value="<%=pj.get(i).getJobCom() %>"><input type="hidden" name="comJob" value="<%=pj.get(i).getJobTitle() %>"><button type="submit" class="btn btn-danger btn2"><i class="fa fa-trash"></i>&nbsp Reject</button></form></td></tr>');
